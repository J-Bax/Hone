...
Hone is an agentic performance optimization system. A **C# harness** (the `.NET 10` solution at `harness-csharp/Hone.slnx`) orchestrates a closed-loop cycle: stress-test the API to find bottlenecks, analyze the measurements with AI to propose an optimization, experiment by implementing the optimization, verify that it actually works (functionally and performance-wise), then publish the results. The target API is treated as a **blackbox** — Hone only requires buildable source, a functional test suite, and k6 stress tests.

## Design Principles

1. **Harness is separate from the target.** The C# harness contains no API-specific logic. It invokes external tools (`dotnet`, `k6`, `copilot`, `git`) via `IProcessRunner` and parses their output. Any API that provides the required contracts can be optimized.
...
The `Hone.Core` project provides the foundational types that every other module depends on:

| C# Type / Interface | Purpose |
|---------------------|---------|
| `HoneConfig` hierarchy | Strongly-typed YAML config loaded by `ConfigLoader` |
| `ConfigLoader` / `ConfigMerger` | YAML deserialization and three-layer config merge |
| `HoneEventBus` / `IHoneEventSink` | Structured event pipeline (console + JSONL sinks) |
| `IHoneEventEmitter` | Emitter interface for all harness components |
| `StringUtils` / `JsonUtils` | Word-boundary truncation, JSON sanitization, code block extraction |
| `MachineInfoCollector` | Machine metadata for experiment records |
| Domain records: `MetricSet`, `ComparisonResult`, `Opportunity`, `QueueItem`, `ExperimentMetadata` | Immutable data shapes shared across all modules |

Configuration is validated at startup by `ConfigValidator` (exposed via `hone validate --target <path>`), which checks paths, ranges, tool availability, and warns about non-obvious setting interactions.
...
flowchart TD
    ...
    subgraph EXPERIMENT["🧪 3. Experiment"]
        E1["Implementer agent"]
        E2["Apply change + build"]
        E1 --> E2
    end
...
All AI agents in the main pipeline (analyst, classifier, implementer) are invoked via a unified runner (`AgentInvoker` in `Hone.Agents.Core`). Agent invocations flow through `IAgentRunner` → `CopilotCliAgentRunner`. The runner ensures:

- **Timeout enforcement** — bounded by `Agents.AgentTimeoutSec` (default 1800s). If an agent hangs, the process is killed.
...
- **Retry with JSON sanitization** — `JsonUtils.SanitizeNaN` strips JavaScript literals; `JsonUtils.ExtractJsonBlock` extracts JSON from markdown fences; retries on parse failure.

For details on the agent pipeline, see [Agent Designs](agent-designs.md).

## Decision Logic

After measuring, `MetricComparer` compares three metrics against the previous experiment:
...
When performance is flat but OS-level resource usage (CPU or working set) decreased, the **efficiency tiebreaker** accepts the experiment — preventing premature stops when there are genuine resource gains. The tiebreaker can be disabled or tuned via `Tolerances.Efficiency` in `config.yaml`.
...
All harness activity is emitted as typed events via `HoneEventBus`. This decouples "what happened" from "how to display it" and enables future TUI/GUI frontends.

```csharp
// All harness components receive IHoneEventEmitter via DI
...
| Sink | Purpose |
|------|---------|
| `ConsoleEventSink` | Timestamped console output with box-drawing support |
| `JsonLogEventSink` | Structured JSONL file with rotation at configurable size (`hone.jsonl`) |

Additional sinks (TUI, webhook, etc.) can be registered at startup via `HoneEventBus.Register(sink)`.
...
8. **Run analyzers** — each `IAnalyzerPlugin` consumes the merged collector data from all passes
...
> For detailed documentation of each agent's role, inputs, outputs, and model configuration, see [Agent Designs](agent-designs.md). For plugin interface contracts, see [Plugin Contracts](plugin-contracts.md). For design ideas on evolving the agent pipeline, see [Future Extensions](future-extensions.md).

### Collector Plugins (`harness/collectors/<name>/`)

Each collector is a self-contained directory implementing `ICollectorPlugin`:

| File | Purpose |
|------|---------|
| `collector.yaml` | Metadata: Name, Group, RequiresAdmin, OverheadImpact, DefaultSettings |
| `<Name>Collector.cs` | `ICollectorPlugin` implementation: `StartAsync`, `StopAsync`, `ExportAsync` |

The `Group` field in `collector.yaml` controls which pass the collector runs in. Collectors with `Group: default` run in every pass. Other collectors in the same group run together; different groups get separate passes.
...
Each analyzer is a self-contained directory implementing `IAnalyzerPlugin`:
...
| `<Name>Analyzer.cs` | `IAnalyzerPlugin` implementation: `AnalyzeAsync` |

Built-in analyzers: `cpu-hotspots` (reads folded CPU stacks from `perfview-cpu`), `memory-gc` (reads GC report from `perfview-gc`)

Each analyzer declares its `RequiredCollectors`. If a required collector's data is not available (e.g., the collector is disabled or failed), the analyzer is automatically skipped with a warning — it does not block other analyzers from running.

### Adding a New Collector Plugin

1. **Create the plugin directory** with the implementation:
...
2. **Define metadata** in `collector.yaml`:
...
3. **Add configuration** under `Diagnostics.CollectorSettings` in `config.yaml`:
...
The `DiagnosticCollectionOrchestrator` automatically discovers and runs all enabled collectors — no orchestrator changes needed.
...
The `DiagnosticAnalysisOrchestrator` automatically discovers and runs all enabled analyzers.
...
2. **Write an agent class** in `Hone.Agents.Loop/` or a new `Hone.Agents.<Domain>/` project:
   - Inject `IAgentRunner` and `IHoneEventEmitter` via constructor
   - Resolve the model from config: per-agent override → `Agents.DefaultModel` → hardcoded fallback
   - Use `JsonUtils.SanitizeNaN` / `JsonUtils.ExtractJsonBlock` for response parsing
   - Save prompt and response artifacts to `experiment-N/`
   - Return a typed result record

3. **Integrate into `HoneLoopRunner`** at the appropriate phase. The current pipeline is: analyst → classifier → implementer. New agents can be inserted between existing stages or added as new phases.

4. **Add model configuration** in `config.yaml` under the `Agents` section.
...
To point Hone at a different API, create a `.hone/config.yaml` in the target repository (see [Adapter Contracts](adapter-contracts.md)) and run:

```sh
hone run --target /path/to/my-api
```

Key settings to configure in the target's `.hone/config.yaml`:
...
| `Api.BaseUrl` | API base URL when running | `http://localhost:0` |
...
The harness C# modules, agent definitions, and decision logic remain unchanged — only the YAML config and lifecycle hooks differ per target.
...
2. **Register the scenario** in `ScaleTest.ScenarioRegistryPath` (`thresholds.json`) with its k6 scenario path.

3. **Set thresholds** in `thresholds.json` for the new scenario.

4. **Establish a baseline** by running `hone baseline --target <path>`, which will capture baseline metrics.

### Custom Exit Conditions and Decision Logic

The accept/reject decision for each experiment is implemented in `MetricComparer` (`Hone.Measurement`). It compares three metrics (p95 latency, requests/sec, error rate) against the previous baseline using configurable tolerances from `config.yaml`.

To modify the decision logic:

- **Adjust thresholds**: Change `Tolerances.MaxRegressionPct` (default 10%) and per-metric absolute thresholds in config
- **Add new metrics**: Extend `MetricComparer` to compare additional metrics (e.g., p99 latency, custom k6 counters) — update `MetricSet` and `ComparisonResult` accordingly
