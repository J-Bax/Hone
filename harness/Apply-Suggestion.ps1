...
.DESCRIPTION
    ...
    code change, commits it along with iteration artifacts and metadata, pushes
    the branch, and opens a pull request for review.
...
try {
    ...
    # If DeferPR, stop here — don't push or create PR
    if ($DeferPR) {
        $result = [ordered]@{
            Success     = $true
...
"@
        ...
        $prUrl = gh pr create `
            --base master `
            --head $branchName `
            --title "hone(iteration-$Iteration): $Description" `
            --body $prBody 2>&1
